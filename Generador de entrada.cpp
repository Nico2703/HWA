...
while(!feof(conf))
{
    ...
    fwrite(&r,sizeof(struct entrada),1,fp);
    fgets(cadena, 5000, conf);
	...
	printf ("\n- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -");
	fread (&r,sizeof(struct entrada),1,fp);
	while (!feof(fp))
			    {
			    printf ("\n %-10s   %-70s            %-40s            %-30s    ",r.fecha,r.descripcion,r.origen,r.cantidad); 
			    fread (&r,sizeof(struct entrada),1,fp);
			    }
fclose(fp);
printf ("\n\n");
